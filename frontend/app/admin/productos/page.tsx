        ...
        <div className="max-w-6xl mx-auto mt-3 flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl font-semibold text-sm transition ${
              vista === "productos" ? "bg-white text-blue-800" : "bg-white/20 text-white hover:bg-white/30"
            }`}
          >
            📦 Productos
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-xl font-semibold text-sm transition ${
              vista === "nuevo" ? "bg-white text-blue-800" : "bg-white/20 text-white hover:bg-white/30"
            }`}
          >
            ➕ Subir Producto
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-xl font-semibold text-sm transition ${
              vista === "categorias" ? "bg-white text-blue-800" : "bg-white/20 text-white hover:bg-white/30"
            }`}
          >
            🏷️ Categorías
          </button>
          ...
          <>
            ...
            <div className="flex gap-3 mb-6 flex-wrap">
              {["Todos", "Ropa", "Calzado", "Electrónica", "Accesorios", "Activo", "Suspendido"].map((f) => (
                <button
                  ...
                  onClick={() => setFiltro(f)}
                  className={`px-4 py-2 rounded-xl font-semibold text-sm transition ${
                    filtro === f ? "bg-blue-700 text-white shadow-md" : "bg-white text-gray-600 border border-gray-200 hover:border-blue-300"
                  }`}
                >
                  {f}
                </button>
            ...
            <div className="flex flex-col gap-3">
              {productosFiltrados.map((producto) => (
                <div key={producto.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">{producto.categoria}</span>
                      <span className={`text-xs font-semibold ${producto.stock === 0 ? "text-red-500" : "text-green-600"}`}>Stock: {producto.stock}</span>
                      <span className="text-xs font-bold text-blue-900">${producto.precio}</span>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <span className={`text-xs font-bold px-3 py-1 rounded-full whitespace-nowrap ${
                      producto.estado === "Activo" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
                    }`}>
                      {producto.estado}
                    </span>
                    <button
                      ...
                      className={`text-xs font-semibold px-3 py-1 rounded-lg transition whitespace-nowrap ${
                        producto.estado === "Activo" ? "bg-red-50 text-red-500 hover:bg-red-100" : "bg-green-50 text-green-600 hover:bg-green-100"
                      }`}
                    >
        ...
        {vista === "nuevo" && (
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="flex flex-col gap-4">
              <div>
                <label className="text-blue-700 font-semibold block mb-1">Nombre del producto *</label>
                <input
                  type="text"
                  placeholder="Ej: Camiseta deportiva"
                  value={nuevoProducto.nombre}
                  onChange={(e) => setNuevoProducto({ ...nuevoProducto, nombre: e.target.value })}
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-700 focus:outline-none focus:border-blue-400"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-blue-700 font-semibold block mb-1">Precio ($) *</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="text-blue-700 font-semibold block mb-1">Stock *</label>
                  <input
                    type="number"
                    placeholder="Ej: 10"
              ...
              <div>
                <label className="text-blue-700 font-semibold block mb-1">Categoría *</label>
                <select
              ...
              <div>
                <label className="text-blue-700 font-semibold block mb-1">Descripción</label>
                <textarea
              ...
              <div>
                <label className="text-blue-700 font-semibold block mb-1">Vendedor asignado</label>
                <select
              ...
              <div>
                <label className="text-blue-700 font-semibold block mb-1">Imagen del producto</label>
                <input
                  type="file"
                  accept="image/*"
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-700 file:mr-4 file:py-1 file:px-4 file:rounded-lg file:border-0 file:bg-blue-700 file:text-white file:font-semibold hover:file:bg-blue-800 transition cursor-pointer"
                />
              </div>
              <div className="flex gap-4 mt-2">
                <button
                  onClick={subirProducto}
                  className="flex-1 bg-blue-700 hover:bg-blue-800 text-white font-bold py-3 rounded-xl transition shadow-lg"
                >
                  Publicar Producto
                </button>
                <button
          ...
          <>
            <div className="flex justify-between items-center mb-6">
              <div className="grid grid-cols-2 gap-4 flex-1 mr-4">
                ...
                <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center">
                  <p className="text-2xl font-bold text-green-600">{categorias.filter(c => c.estado === "Activa").length}</p>
                  <p className="text-gray-400 text-sm">Activas</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarFormCategoria(!mostrarFormCategoria)}
                className="bg-blue-700 hover:bg-blue-800 text-white font-bold px-4 py-3 rounded-xl transition shadow-md whitespace-nowrap"
              >
                + Nueva Categoría
              </button>
            </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 bg-blue-700 hover:bg-blue-800 text-white font-bold py-3 rounded-xl transition"
                    >
                      Guardar Categoría
                    </button>
                    <button
            ...
            <div className="flex flex-col gap-3">
              {categorias.map((categoria) => (
                <div key={categoria.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="font-bold text-gray-800 text-lg">{categoria.nombre}</p>
                    <p className="text-gray-400 text-sm">{categoria.descripcion}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <span className={`text-xs font-bold px-3 py-1 rounded-full whitespace-nowrap ${
                      categoria.estado === "Activa" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
                    }`}>
                      {categoria.estado}
                    </span>
                    <div className="flex gap-2">
                      <button
                        ...
                        className={`text-xs font-semibold px-3 py-1 rounded-lg transition whitespace-nowrap ${
                          categoria.estado === "Activa" ? "bg-red-50 text-red-500 hover:bg-red-100" : "bg-green-50 text-green-600 hover:bg-green-100"
                        }`}
                      >
                      ...
                      <button
                        ...
                        className="text-xs font-semibold px-3 py-1 rounded-lg transition whitespace-nowrap bg-gray-100 text-gray-500 hover:bg-gray-200"
                      >
                        🗑️
                      </button>
                    </div>
